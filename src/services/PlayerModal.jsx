      ...
      <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-slate-900 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-white/20">
          ...
          <div className="relative bg-gradient-to-r from-cyan-500/20 to-purple-500/20 p-4 md:p-6 border-b border-white/10">
            ...
            {/* Initial View - Large Image with Key Stats */}
            {!activeView && (
              <div className="flex flex-col items-center text-center space-y-6 py-8">
                <div className="w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden border-4 border-white/20 flex-shrink-0 shadow-2xl">
                  <img
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 text-gray-300">
                    <div className="flex flex-col items-center space-y-2 bg-black/30 rounded-lg p-4 border border-white/10">
                      <Target className="w-6 h-6 text-cyan-400" />
                      <span className="text-sm text-gray-400">Position</span>
                      <span className="font-semibold text-white">{player.bioData.position}</span>
                    </div>
                    <div className="flex flex-col items-center space-y-2 bg-black/30 rounded-lg p-4 border border-white/10">
                      <Calendar className="w-6 h-6 text-purple-400" />
                      <span className="text-sm text-gray-400">Age</span>
                      <span className="font-semibold text-white">{calculateAge(player.bioData.dateOfBirth)} years</span>
                    </div>
                    <div className="flex flex-col items-center space-y-2 bg-black/30 rounded-lg p-4 border border-white/10">
                      <Trophy className="w-6 h-6 text-green-400" />
                      <span className="text-sm text-gray-400">Int'l Caps</span>
                      <span className="font-semibold text-white">{player.bioData.internationalCaps}</span>
                    </div>
                    <div className="flex flex-col items-center space-y-2 bg-black/30 rounded-lg p-4 border border-white/10">
                      <Shirt className="w-6 h-6 text-orange-400" />
                      <span className="text-sm text-gray-400">Jersey</span>
                      <span className="font-semibold text-white">{player.bioData.jersey || 0}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {/* Compact View - When Overview or Stats is Active */}
            {activeView && (
              <>
